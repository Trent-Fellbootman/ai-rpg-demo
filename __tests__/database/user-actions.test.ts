...
import { createUser, getUserFromEmail } from '@/app/lib/data/database-actions/user-actions';
...
describe('User Actions', () => {
  ...
  test.concurrent('should throw conflict error when creating a user with an existing email', async () => {
    const email = `testuser-${uuidv4()}@example.com`;
    const hashedPassword = 'hashedpassword';
    const name = 'Test User';
    ...
    await expect(createUser(email, hashedPassword, name)).rejects.toThrowError(DatabaseError);
  ...
  test.concurrent('should throw not found error when retrieving a non-existing user', async () => {
